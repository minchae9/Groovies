...
for movie_id in movie_id_list:
    request_url = tmdb_helper.get_request_url(method=f'/movie/{movie_id}/videos', language='ko-KR')
    ...
    for info in infos:
        if info['type'] == 'Trailer':
            item = {}
            item['model'] = "movies.movie"
            item['pk'] = idx
            item['fields'] = {}
            item['fields']['trailer_key'] = info['key']
            trailers.append(item)
            idx += 1
            break
file_path = 'movies/fixtures/trailers.json'
with open(file_path, 'w') as outfile:
    json.dump(trailers, outfile, indent=4)
